...
cache_glyph :: proc(
    ...
    if rast.width > 0 && rast.height > 0 {
        region, packed := shelf_pack(
            atlas, u32(rast.width), u32(rast.height),
        )
        if packed {
            // Blit into staging buffer
...
shelf_pack :: proc(
    ...
    append(&atlas.shelves, Atlas_Shelf {
        y      = y_start,
        height = h,
        x_used = w + padding,
    })
