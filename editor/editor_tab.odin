...
render_tab_bar :: proc(
	...
	for tab, idx in bar.tabs {
		...
		text_x := x_offset + 10
		text_y := y_offset + (f32(bar.height) - f32(bar.text_renderer.line_height)) / 2
		...
		// text_width := measure_text_width(bar.text_renderer, title_text)
		...
		// 		test_width := measure_text_width(bar.text_renderer, test_text)
	...
	_ = sdl.SetRenderDrawColor(renderer, 0x45, 0x45, 0x45, 0xFF)
	_ = sdl.RenderLine(renderer, 0, y_offset + f32(bar.height), f32(window_w), y_offset + f32(bar.height))
}
