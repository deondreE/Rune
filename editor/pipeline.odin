...
create_pipeline :: proc(
	...
	attributes := [3]vk.VertexInputAttributeDescription {
        {location = 0, binding = 0, format = .R32G32_SFLOAT, offset = u32(offset_of(Text_Vertex, pos))},
        {location = 1, binding = 0, format = .R32G32_SFLOAT, offset = u32(offset_of(Text_Vertex, uv))},
        {location = 2, binding = 0, format = .R32G32B32A32_SFLOAT, offset = u32(offset_of(Text_Vertex, color))},
    }
	...
	if kind == .Text {
		...
		if vk.CreateDescriptorSetLayout(
			ctx.device, &set_layout_info, nil, &pipe.set_layout
		) != .SUCCESS {
			return pipe, false
		}
		...
		if vk.CreateDescriptorPool(
			ctx.device, &pool_info, nil, &pipe.descriptor_pool,
		) != .SUCCESS {
			return pipe, false
		}
	...
	layout_info := vk.PipelineLayoutCreateInfo {
		...
		pSetLayouts = p_set_layout,
		pushConstantRangeCount = 1,
		pPushConstantRanges = &push_range,
	}

	if vk.CreatePipelineLayout(
		ctx.device, &layout_info, nil, &pipe.layout,
	) != .SUCCESS {
		return pipe, false
	}
	...
	if vk.CreateGraphicsPipelines(
		ctx.device, {}, 1, &pipeline_info, nil, &pipe.pipeline,	
	) != .SUCCESS {
		return pipe, false
	}
